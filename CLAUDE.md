# CLAUDE.md — DataScience Project Workspace

## Identity

- **Workspace**: `/workspaces/`
- **Environment**: Dev Container (Ubuntu 22.04, Miniconda + PyTorch CPU, user=vscode)
...
├── PROJECT.md                      # Domain context (DS template)
...
│   ├── Dockerfile                  # Image (Node.js + Serena + Miniconda + DS)
...
이 저장소는 **DataScience 도메인 템플릿**입니다. Tier 1 베이스에서 파생되었습니다.

| Tier | 역할 | 포함 |
|------|------|------|
| **Tier 1** | 베이스 템플릿 (claude-devcontainer) | 13 agents, 12 hooks, 8 skills, DevContainer 인프라 |
| **DS Domain** | Tier 1 + Data Science (이 저장소) | Miniconda, PyTorch CPU, Jupyter, DS 패키지 |
...
- **Ports**: `.devcontainer/.env`에서 관리 (PORT_APP, PORT_API, PORT_DB, PORT_EXTRA=8888)
- **Runtime 격리 (3-tier)**: Claude Code(Node.js v20) / Serena(시스템 python3) / DS(conda env ds)
